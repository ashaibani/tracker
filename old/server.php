...
class Server
{
    ...
    private function createSocket()
    {
        if (!$this->socket = socket_create(AF_INET, SOCK_STREAM, 0)) {
            echo "[Smartwatch Tracker] Failed to create socket: " . socket_strerror($this->socket) . "\n";
            exit();
        }

        if (!$ret = socket_bind($this->socket, $this->addr, $this->port)) {
            echo "[Smartwatch Tracker] Failed to bind socket: " . socket_strerror($ret) . "\n";
            exit();
        }

        if (!$ret = socket_listen($this->socket, 0)) {
            echo "[Smartwatch Tracker] Failed to listen to socket: " . socket_strerror($ret) . "\n";
            exit();
        }
    ...
    function listen($maxLength = 2000)
    {
        ...
        while (true) {
            ...
            if ($client === false) {
                ...
                $smartWatch = new Smartwatch($client, 1, $address.":".$port, $this->debug);
